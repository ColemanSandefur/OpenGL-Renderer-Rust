...
/// A simple texture loader
///
/// A grouping of functions that make loading opengl textures easier. Be it straight
/// from the file system, or from a buffer this should (eventually) have everything that you need
/// to load a texture.
...
impl TextureLoader {
    ...
    /// Converts the buffer into an opengl texture.
    ///
    /// The buffer should be arranged with the pattern of red byte, green byte, blue byte. It will
    /// fail if the buffer doesn't have enough content to fill the image. The buffer should be of
    /// length (width * height * 3).
    pub fn from_memory_rgb8(facade: &impl Facade, buffer: &[u8], width: u32, height: u32) -> Result<Texture2d, Box<dyn Error>>{
        let buffer_grouped = buffer.chunks_exact(3).map(|chunk| {
            return (chunk[0], chunk[1], chunk[2])
        });

        Ok(
            Texture2d::with_mipmaps(
                facade,
    ...
    /// Converts the buffer into an opengl texture.
    ///
    /// The buffer should be arranged with the pattern of red float, green float, blue float. It will
    /// fail if the buffer doesn't have enough content to fill the image. The buffer should be of
    /// length (width * height * 3).
    pub fn from_memory_rgbf32(facade: &impl Facade, buffer: &[f32], width: u32, height: u32) -> Result<Texture2d, Box<dyn Error>> {
        let buffer_grouped = buffer.chunks_exact(3).map(|chunk| {
            return (chunk[0], chunk[1], chunk[2])
        });
