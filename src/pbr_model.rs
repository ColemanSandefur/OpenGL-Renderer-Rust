...
impl PbrModel {
    pub fn load_from_fs2(path: PathBuf, facade: &impl Facade, material: PBR) -> Self {
        //let scene = Scene::from_file("ship.fbx", vec![PostProcess::Triangulate]).unwrap();

        //println!(
        //"{}, {}",
        //scene.meshes[0].normals.len(),
        //scene.meshes[0].vertices.len(),
        //);

        //println!("{:#?}", scene.materials[0].properties);
        ...
        for model in models {
            ...
            if let Some(material_index) = model.mesh.material_id {
                ...
                // Definitely shouldn't do this but it will do for now
